...
# Run the confirmation script and then start the application with Gunicorn
# Gunicorn manages worker lifecycle: recycling, timeouts, and process health
CMD ["/bin/bash", "-c", "/app/scripts/confirm_env.sh && gunicorn app.main:app -k uvicorn.workers.UvicornWorker --bind 0.0.0.0:8000 --workers 2 --timeout 180 --max-requests 1000 --max-requests-jitter 50 --keep-alive 30 --access-logfile - --error-logfile -"]
